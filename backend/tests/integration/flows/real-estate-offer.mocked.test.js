/**
 * Mocked version of Real Estate Offer Flow test
...
describe('Real Estate Offer Flow (Mocked)', () => {
    ...
    it('should create a real estate offer with collateral and documents', async () => {
        ...
        try {
            const response = await request(app)
                ...
                .field('asset_code', 'RES02') // Different asset code to avoid collision
                .field('offer_name', 'Residencial Alphaville Mocked')
                .field('description', 'Tokenização de imóvel residencial (mocked)')
            ...
            assert.strictEqual(response.body.data.assetCode, 'RES02');
    ...
    it('should retrieve offer details with collateral info', async () => {
        ...
        const offer = response.body.data.find(o => o.assetCode === 'RES02');
