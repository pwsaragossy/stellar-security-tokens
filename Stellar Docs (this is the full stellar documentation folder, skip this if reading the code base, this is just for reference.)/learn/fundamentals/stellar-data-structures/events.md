...
## How are events emitted?[](#how-are-events-emitted "Direct link to How are events emitted?")
...
### ContractEvent[](#contractevent "Direct link to ContractEvent")

An event's topics don't have to be made of the same type: you can mix different types.

An event also contains a data object of any value or type, including [custom types](/docs/learn/fundamentals/contract-development/types/custom-types.md) defined by contracts using `#[contracttype]`:
...
### TransactionMetaV3[](#transactionmetav3 "Direct link to TransactionMetaV3")
...
### TransactionMetaV4[](#transactionmetav4 "Direct link to TransactionMetaV4")
...
## What are diagnosticEvents?[](#what-are-diagnosticevents "Direct link to What are diagnosticEvents?")
...
### fn\_return[](#fn_return "Direct link to fn_return")

The `fn_return` diagnostic event is emitted when a contract call completes and contains -

* Topics
  1. The symbol "fn\_return".
  2. A symbol containing the name of the function that is about to return.
* Data
  1. The value returned by the contract function.

### When should diagnostic events be enabled?[](#when-should-diagnostic-events-be-enabled "Direct link to When should diagnostic events be enabled?")
...
## Tracking the movement of value[](#tracking-the-movement-of-value "Direct link to Tracking the movement of value")
...
## Reading events[](#reading-events "Direct link to Reading events")

You can use the [`getEvents`](/docs/data/apis/rpc/api-reference/methods/getEvents.md) endpoint of any RPC service to fetch and filter events by type, contract, and topic.

> **Warning:** Events are ephemeral: RPC providers typically only keep short chunks (less than a week) of history around.

To learn more about working with events, take a look at the [events guides](/docs/build/guides/events.md) and [this example contract](/docs/build/smart-contracts/example-contracts/events.md).

For a quick high-level demonstration, though, we'll use the [TypeScript SDK](/docs/tools/sdks.md) to infinitely fetch all `transfer` events (defined by the [Soroban Token Interface](https://stellar.org/protocol/sep-41#interface)) involving the [XLM contract](https://stellar.expert/explorer/testnet/contract/CDLZFC3SYJYDZT7K67VZ75HPJVIEUVNIXF47ZG2FB2RMQQVU2HHGCYSC) and display them in a human-friendly format.
...
You can also leverage RPC's alternate [XDR encoding formats](/docs/data/apis/rpc/api-reference/structure/data-format.md) like JSON to see human-readable events from the command-line directly, for example by passing `xdrFormat: "json"` as an additional parameter to the `getEvents` [example](/docs/data/apis/rpc/api-reference/methods/getEvents.md).