...
Learn to write your first Stellar smart contract in the [Getting Started Guide](/docs/build/smart-contracts/getting-started/setup.md).

## Smart Contract in WebAssembly (Wasm)[](#smart-contract-in-webassembly-wasm "Direct link to Smart Contract in WebAssembly (Wasm)")
...
## Contract Instances[](#contract-instances "Direct link to Contract Instances")
...
In addition to the ledger entries that are created during the contract upload/deploy process, each contract can create and access its own set of ledger entries. These ledger entries (as well as the contract code and the contract instance ledger entries) are subject to [state archival](/docs/learn/fundamentals/contract-development/storage/state-archival.md) lifetimes, or TTL (time-to-live) behavior. Each storage type has distinct fee structures, TTL (time-to-live) behavior, and is designed to store specific types of data. Using the incorrect storage can lead to logical errors in your application, in addition to impacting costs.
...
## Invoke Contracts[](#invoke-contracts "Direct link to Invoke Contracts")
...
## Contract Info[](#contract-info "Direct link to Contract Info")
...
![Contract Info Meta](/assets/images/contract-info-meta-b91b83d11132987b50da96be78a86da1.png) ![Contract Info Spec](/assets/images/contract-info-spec-ff2d46506b194102bd068ae0f21c1fb3.png)

*Explore contract info at [Stellar Lab's Contract Explorer](https://lab.stellar.org/smart-contracts/contract-explorer).*

### Environment Meta[](#environment-meta "Direct link to Environment Meta")
...
### Contract Meta[](#contract-meta "Direct link to Contract Meta")
...
### Contract Spec[](#contract-spec "Direct link to Contract Spec")
...
#### Fully Typed Contract Definitions[](#fully-typed-contract-definitions "Direct link to Fully Typed Contract Definitions")
...
#### Comparison to Ethereum ABI[](#comparison-to-ethereum-abi "Direct link to Comparison to Ethereum ABI")
...
#### Generating contract specs[](#generating-contract-specs "Direct link to Generating contract specs")
...
Sometimes, you may not have access to the contract's source code or the ability to compile it. In such cases, you must use the [`stellar contract fetch`](/docs/tools/cli/stellar-cli.md) command to download the contract's Wasm binary and generate the spec.

Finally, we use the [`stellar bindings`](/docs/tools/cli/stellar-cli.md) command to generate the contract spec from the Wasm binary.
...
> **Note:** Having read the introduction of a Stellar contract, let's try to write a smart contract by following the [Getting Started guide](/docs/build/smart-contracts/getting-started.md).