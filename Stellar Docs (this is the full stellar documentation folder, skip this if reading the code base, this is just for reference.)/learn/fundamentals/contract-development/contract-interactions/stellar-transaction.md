...
## Example SDK Usage[](#example-sdk-usage "Direct link to Example SDK Usage")

Some (but not all yet) of the Stellar SDKs have functions built-in to handle most of the process of building a Stellar transaction to interact with a Soroban smart contract. Below, we demonstrate in JavaScript and Python how to build and submit a Stellar transaction that will invoke an instance of the [increment example](/docs/build/smart-contracts/getting-started/storing-data.md) smart contract.

* JavaScript
* Python
* Java

> **Tip:** The existing [JavaScript SDK](https://github.com/stellar/js-stellar-sdk) now incorporates all of the elements needed for Soroban. All you need to do is install it using your preferred package manager.
...
> **Tip:** The [`py-stellar-base`](https://stellar-sdk.readthedocs.io/en/soroban/) Python SDK has stable support for interacting with Soroban smart contracts as of [v9.0.0](https://github.com/StellarCN/py-stellar-base/releases/tag/9.0.0).
...
> **Tip:** [java-stellar-sdk](https://github.com/stellar/java-stellar-sdk) provides support for Soroban, please visit the project homepage for more information.
...
## XDR Usage[](#xdr-usage "Direct link to XDR Usage")

Stellar supports invoking and deploying contracts with a new operation named `InvokeHostFunctionOp`. The [`stellar-cli`](/docs/build/smart-contracts/getting-started/setup.md) abstracts these details away from the user, but not all SDKs do yet. If you're building a dapp you'll probably find yourself building the XDR transaction to submit to the network.

The `InvokeHostFunctionOp` can be used to perform the following Soroban operations:

* Invoke contract functions.
* Upload Wasm of the new contracts.
* Deploy new contracts using the uploaded Wasm or built-in implementations (this currently includes only the [token contract](/docs/tokens/stellar-asset-contract.md)).
...
### InvokeHostFunctionOp[](#invokehostfunctionop "Direct link to InvokeHostFunctionOp")
...
##### JavaScript Usage[](#javascript-usage "Direct link to JavaScript Usage")

Each of these variations of host function invocation has convenience methods in the [JavaScript SDK](/docs/tools/sdks/client-sdks.md):
...
* [`Operation.createStellarAssetContract`](https://stellar.github.io/js-stellar-sdk/Operation.html#.createStellarAssetContract) and [`Operation.createCustomContract`](https://stellar.github.io/js-stellar-sdk/Operation.html#.createCustomContract) are abstractions to instantiate contracts: the former is for wrapping an existing [Stellar asset](/docs/tokens/how-to-issue-an-asset.md) into a smart contract and the latter is for deploying your own contract.
...
#### Authorization Data[](#authorization-data "Direct link to Authorization Data")

Soroban's [authorization framework](/docs/learn/fundamentals/contract-development/authorization.md) provides a standardized way for passing authorization data to the contract invocations via `SorobanAuthorizationEntry` structures.
...
Building `SorobanAuthorizedInvocation` trees may be simplified by using the recording auth mode in Soroban's `simulateTransaction` mechanism (see the [docs](/docs/learn/fundamentals/contract-development/contract-interactions/transaction-simulation.md) for more details).

##### Stellar Account Signatures[](#stellar-account-signatures "Direct link to Stellar Account Signatures")

`signatureArgs` format is user-defined for the [custom accounts](/docs/learn/fundamentals/contract-development/authorization.md), but it is protocol-defined for the Stellar accounts.
...
##### JavaScript Usage[](#javascript-usage-1 "Direct link to JavaScript Usage")
...
* If you, instead, want to *build* an authorization entry from scratch rather than relying on simulation, you can use [`authorizeInvocation`](https://stellar.github.io/js-stellar-sdk/global.html#authorizeInvocation), which will build the structure with the appropriate fields.
...
The simplest method to determine the values in `SorobanResources` and `refundableFee` is to use the [`simulateTransaction` mechanism](/docs/learn/fundamentals/contract-development/contract-interactions/transaction-simulation.md).

#### JavaScript Usage[](#javascript-usage-2 "Direct link to JavaScript Usage")
