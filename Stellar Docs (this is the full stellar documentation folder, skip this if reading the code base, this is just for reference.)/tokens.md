...
1. Assets issued by Stellar accounts (`G...` addresses) and their built-in [Stellar Asset Contract (SAC)](/docs/tokens/stellar-asset-contract.md) implementation, and
2. [Contract tokens](/docs/tokens/token-interface.md) issued by a deployed WASM contract (`C...` addresses).
...
## Issuing assets on Stellar[](#issuing-assets-on-stellar "Direct link to Issuing assets on Stellar")

Stellar has first-class support for asset tokenization  issuing an asset can be done using a [built-in transaction](/docs/tokens/quickstart.md) without the development of a smart contract.
...
* Benefit from [anchors](/docs/learn/fundamentals/anchors.md), the bridges between the Stellar network and traditional financial systems. Explore the global [Stellar anchor directory](https://anchors.stellar.org/) for further details.
* Give the issuer granular control over asset management with features that allow the issuer to [name the asset](/docs/tokens/control-asset-access.md), [determine access control](/docs/tokens/control-asset-access.md), [limit asset supply](/docs/tokens/control-asset-access.md), [publish asset information](/docs/tokens/publishing-asset-info.md), and [ensure compliance](/docs/tokens/anatomy-of-an-asset.md).

> **Note:** Note that while these items are also possible with smart contract tokens, it is more work to build the token contract rather than using the already-implemented features of Stellar asset tokens.
...
### Stellar Asset Contract[](#stellar-asset-contract "Direct link to Stellar Asset Contract")
...
Read more about the SAC [here](/docs/tokens/stellar-asset-contract.md).

Learn how to deploy a Stellar Asset Contract for an asset in [this How-To Guide](/docs/tools/cli/cookbook/deploy-stellar-asset-contract.md).
...
## Contract tokens[](#contract-tokens "Direct link to Contract tokens")

If you have a unique use case where the capabilities Stellar Assets are not sufficient, you can create a contract token that implements the [token interface](/docs/tokens/token-interface.md). The token interface specifies the functions and events a contract must implement to be compatible with applications that use tokens.
...
> **Note:** Smart contracts cannot use Stellar assets unless that Stellar asset has a deployed SAC. Anyone can deploy the SAC for a Stellar asset to its reserved address.
...
* [Issue an asset tutorial](/docs/tokens/how-to-issue-an-asset.md)
* [Stellar Asset Contract](/docs/tokens/stellar-asset-contract.md)
* [Token Interface](/docs/tokens/token-interface.md)