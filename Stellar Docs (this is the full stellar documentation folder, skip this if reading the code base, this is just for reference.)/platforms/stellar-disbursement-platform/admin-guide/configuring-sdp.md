...
* Configurations that are tagged with ð are sensitive and should be stored securely.
...
### Operational Configuration[](#operational-configuration "Direct link to Operational Configuration")
...
* `CRASH_TRACKER_TYPE` - The crash tracker type to use. Options: "SENTRY", "DRY\_RUN". Default: "DRY\_RUN".
* `SENTRY_DSN` - ð The DSN (client key) of the Sentry project. If not provided, Sentry will not be used.
* `ENVIRONMENT` - The environment where the application is running. Example: "development", "staging", "production". Default: "development".
* `BASE_URL` - The SDP backend server's base URL. Default: "<http://localhost:8000>". Tenant-specific URLs will be configured during the [tenant provisioning process](/docs/platforms/stellar-disbursement-platform/admin-guide/advanced-configration.md).
* `SDP_UI_BASE_URL` - The SDP UI/dashboard Base URL used to send the invitation link when a new user is created. Tenant-specific URLs will be configured during the [tenant provisioning process](/docs/platforms/stellar-disbursement-platform/admin-guide/advanced-configration.md).
...
* `DATABASE_URL` - ð The connection string for the PostgreSQL database. Format is `postgres://username:password@host:port/database?sslmode=disable`. Default: "postgres://localhost:5432/sdp?sslmode=disable".
...
### Messaging Configuration[](#messaging-configuration "Direct link to Messaging Configuration")

Messaging Configuration allows configuring the messaging service used to send messages to recipients and sdp dashboard users. The default configuration is set to "DRY\_RUN" which means no messages will be sent and the messages will be logged to the console. This is recommended for testing purposes only.

* `EMAIL_SENDER_TYPE`: The messenger type used to send invitations to new dashboard users. Options: "DRY\_RUN", "TWILIO\_EMAIL", "AWS\_EMAIL". Default: "DRY\_RUN".
* `SMS_SENDER_TYPE`: The messenger type used to send SMS messages to recipients. Options: "DRY\_RUN", "TWILIO\_SMS", "TWILIO\_WHATSAPP", "AWS\_SMS". Default: "DRY\_RUN".

#### AWS Configuration[](#aws-configuration "Direct link to AWS Configuration")

The following configurations are required when using AWS SES or SNS to send emails or SMS messages.

* `AWS_ACCESS_KEY_ID` - ð The AWS access key ID.
* `AWS_REGION` - The AWS region where the SES service is available.
* `AWS_SECRET_ACCESS_KEY` - ð The AWS secret access key.
* `AWS_SES_SENDER_ID` - The email that AWS SES will use as the sender when sending emails. Required when `EMAIL_SENDER_TYPE` is set to "AWS\_EMAIL".
* `AWS_SNS_SENDER_ID` - The sender ID to use when sending SMS messages using AWS SNS. Required when `SMS_SENDER_TYPE` is set to "AWS\_SMS".

#### Twilio Configuration[](#twilio-configuration "Direct link to Twilio Configuration")

The following configurations are required when `SMS_SENDER_TYPE=TWILIO_SMS`.

* `TWILIO_ACCOUNT_SID` - ð The Twilio account SID.
* `TWILIO_AUTH_TOKEN` - ð The Twilio auth token.
...
* `TWILIO_SENDGRID_API_KEY` - ð The API key for the Twilio SendGrid (email) service.
* `TWILIO_SENDGRID_SENDER_ADDRESS` - The email address used to send emails via Twilio SendGrid.

#### General Messaging Configuration[](#general-messaging-configuration "Direct link to General Messaging Configuration")
...
### Stellar Configuration[](#stellar-configuration "Direct link to Stellar Configuration")
...
* `SEP10_SIGNING_PRIVATE_KEY` - ð The private key of the Stellar account that signs the SEP-10 transactions. It's also used to sign URLs.
...
#### Channel Accounts Configuration[](#channel-accounts-configuration "Direct link to Channel Accounts Configuration")

The following configuration is required for using the [channel-accounts CLI](/docs/platforms/stellar-disbursement-platform/admin-guide/cli-manual.md) to manage channel accounts.

* `CHANNEL_ACCOUNT_ENCRYPTION_PASSPHRASE` - ð A Stellar ed25519 secret key (starting with `S`) used to encrypt/decrypt the channel accounts' private keys. When not set, it will default to the value of `DISTRIBUTION_SEED`.

#### Distribution Accounts Configuration[](#distribution-accounts-configuration "Direct link to Distribution Accounts Configuration")
...
* `DISTRIBUTION_ACCOUNT_ENCRYPTION_PASSPHRASE` - ð A Stellar ed25519 secret key (starting with `S`) used to encrypt/decrypt the in-memory distribution accounts' private keys.
...
* `DISTRIBUTION_SEED` - ð The private key of the HOST's Stellar distribution account. Used to disburse funds, create channel accounts and tenant distribution accounts.
...
* `SEP24_JWT_SECRET` - ð The secret used to sign the JWT token for SEP-24 transactions. This secret is used during the receiver wallet registration flow.

#### Dashboard Authentication Configuration[](#dashboard-authentication-configuration "Direct link to Dashboard Authentication Configuration")
...
* `EC256_PRIVATE_KEY` - ð The EC256 Private Key used to sign the authentication token. This EC key needs to be at least as strong as prime256v1 (P-256).
...
#### Recaptcha Configuration[](#recaptcha-configuration "Direct link to Recaptcha Configuration")
...
* `RECAPTCHA_SITE_SECRET_KEY` - ð The reCAPTCHA site secret key used to validate reCAPTCHA responses.

### Background Jobs Configuration[](#background-jobs-configuration "Direct link to Background Jobs Configuration")
...
### Multi-tenancy Configuration[](#multi-tenancy-configuration "Direct link to Multi-tenancy Configuration")
...
* `SINGLE_TENANT_MODE`: When set to `"true"`, it enables the single-tenant mode, which is useful for local development or single-tenant setups. In addition to set it to true, you'll need to configure the default tenant by calling the [`POST /tenants/default-tenant`](/docs/platforms/stellar-disbursement-platform/api-reference/default-tenant.md) request.
...
### Bridge Integration Configuration[](#bridge-integration-configuration "Direct link to Bridge Integration Configuration")
...
* `BRIDGE_API_KEY` - ð The API key for the bridge integration. Required if `ENABLE_BRIDGE_INTEGRATION` is set to true.
...
### Operational Configuration[](#operational-configuration-1 "Direct link to Operational Configuration")
...
### Database Configuration[](#database-configuration-1 "Direct link to Database Configuration")
...
### Stellar Configuration[](#stellar-configuration-1 "Direct link to Stellar Configuration")
...
#### Channel Accounts Configuration[](#channel-accounts-configuration-1 "Direct link to Channel Accounts Configuration")
...
* `CHANNEL_ACCOUNT_ENCRYPTION_PASSPHRASE` - ð A Stellar ed25519 secret key (starting with `S`) used to encrypt/decrypt the channel accounts' private keys. When not set, it will default to the value of the 'DISTRIBUTION\_SEED' option.

#### Distribution Accounts Configuration[](#distribution-accounts-configuration-1 "Direct link to Distribution Accounts Configuration")
...
### General Configuration[](#general-configuration-1 "Direct link to General Configuration")
...
* `SINGLE_TENANT_MODE` - When set to `"true"`, it enables the single-tenant mode, which is useful for local development or single-tenant setups. In addition to set it to true, you'll need to configure the default tenant by calling the [`POST /tenants/default-tenant`](/docs/platforms/stellar-disbursement-platform/api-reference/default-tenant.md) request. Default: "false".