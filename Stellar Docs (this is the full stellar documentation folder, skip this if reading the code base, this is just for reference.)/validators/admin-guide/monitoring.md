...
> **Info:** [**Stellarbeat**](https://www.stellarbeat.io), a community-run monitoring dashboard, can also be useful for viewing network health as a whole. You should keep a very close eye on your own node(s) using some of the tools suggested on this page, but monitoring performance of all the network's nodes can also be useful to understand how they all interact with each other.
...
## General Node Information[](#general-node-information "Direct link to General Node Information")
...
* `network` the [network passphrase]([Networks section](/docs/networks.md)) for the network this core instance is using
...
## Peer Information[](#peer-information "Direct link to Peer Information")
...
## Overlay Topology Survey[](#overlay-topology-survey "Direct link to Overlay Topology Survey")
...
### Overlay Survey Script[](#overlay-survey-script "Direct link to Overlay Survey Script")
...
#### Attaching to a Running Survey[](#attaching-to-a-running-survey "Direct link to Attaching to a Running Survey")
...
#### More Survey Script Options[](#more-survey-script-options "Direct link to More Survey Script Options")
...
### Example Survey Command Using HTTP Endpoints[](#example-survey-command-using-http-endpoints "Direct link to Example Survey Command Using HTTP Endpoints")
...
In this example, we have three nodes `GBBN`, `GDEX`, and `GBUI` (we'll refer to them by the first four letters of their public keys). We will execute the commands below from `GBUI`, and note that `GBBN` has `SURVEYOR_KEYS=["$self"]` in its config file, so `GBBN` will not relay or respond to any survey messages.
...
## Quorum Health[](#quorum-health "Direct link to Quorum Health")
...
### Quorum Set Diagnostics[](#quorum-set-diagnostics "Direct link to Quorum Set Diagnostics")
...
### Per-node Quorum-set Information[](#per-node-quorum-set-information "Direct link to Per-node Quorum-set Information")
...
* too many validators missing (down or without a good connectivity), solutions are:
  + [adjust your quorum set](/docs/validators/admin-guide/configuring.md) (thresholds, grouping, etc.) based on the nodes that are not missing
  + try to get a [better connectivity path](/docs/validators/admin-guide/configuring.md) to the missing validators
...
### Transitive Closure Summary Information[](#transitive-closure-summary-information "Direct link to Transitive Closure Summary Information")
...
### Detailed Transitive Quorum Analysis[](#detailed-transitive-quorum-analysis "Direct link to Detailed Transitive Quorum Analysis")
...
## Using Prometheus[](#using-prometheus "Direct link to Using Prometheus")
...
### Install a Prometheus Server Within your Infrastructure[](#install-a-prometheus-server-within-your-infrastructure "Direct link to Install a Prometheus Server Within your Infrastructure")
...
### Install the `stellar-core-prometheus-exporter`[](#install-the-stellar-core-prometheus-exporter "Direct link to install-the-stellar-core-prometheus-exporter")
...
### Point Prometheus to stellar-core-prometheus-exporter[](#point-prometheus-to-stellar-core-prometheus-exporter "Direct link to Point Prometheus to stellar-core-prometheus-exporter")
...
#### Using Service Discovery (EC2)[](#using-service-discovery-ec2 "Direct link to Using Service Discovery (EC2)")
...
### Create Alerting Rules[](#create-alerting-rules "Direct link to Create Alerting Rules")

Once Prometheus scrapes metrics we can add alerting rules. Recommended rules are [**here**](https://github.com/stellar/packages/blob/master/docs/stellar-core-alerting.rules) (require Prometheus 2.0 or later). Copy rules to */etc/prometheus/stellar-core-alerting.rules* on the Prometheus server and add the following to the prometheus configuration file to include the file:
...
### Configure Notifications Using Alertmanager[](#configure-notifications-using-alertmanager "Direct link to Configure Notifications Using Alertmanager")
...
### Useful Exporters[](#useful-exporters "Direct link to Useful Exporters")
...
### Visualize Metrics Using Grafana[](#visualize-metrics-using-grafana "Direct link to Visualize Metrics Using Grafana")
