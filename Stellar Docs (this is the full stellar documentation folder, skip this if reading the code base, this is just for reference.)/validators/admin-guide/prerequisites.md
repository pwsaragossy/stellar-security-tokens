...
You can install Stellar Core a [number of different ways](/docs/validators/admin-guide/installation.md), and once you do, you can [configure](/docs/validators/admin-guide/configuring.md) it to participate in the network on a several [different levels](/docs/validators.md): it can be either a Basic Validator or a Full Validator. No matter how you install Stellar Core or what kind of node you run, however, you need to set up and connect to the peer-to-peer network and store the state of the ledger in a SQL [database](/docs/validators/admin-guide/configuring.md).

## Hardware Requirements[](#hardware-requirements "Direct link to Hardware Requirements")

> **Info:** CPU, RAM, Disk and network depends on network activity. If you decide to collocate certain workloads, you will need to take this into account.
...
## Stellar Network Access[](#stellar-network-access "Direct link to Stellar Network Access")
...
### Inbound[](#inbound "Direct link to Inbound")

A Stellar Core node needs to allow all IPs to connect to its `PEER_PORT` over TCP. You can specify a port when you [configure](/docs/validators/admin-guide/configuring.md) Stellar Core, but most people use the default, which is **11625**.

### Outbound[](#outbound "Direct link to Outbound")
...
## Internal System Access[](#internal-system-access "Direct link to Internal System Access")
...
### Inbound[](#inbound-1 "Direct link to Inbound")

* Stellar Core exposes an *unauthenticated* HTTP endpoint on its `HTTP_PORT`. You can specify a port when you [configure](/docs/validators/admin-guide/configuring.md) Stellar Core, but most people use the default, which is **11626**.
...
* It's also used to query Stellar Core [info](/docs/validators/admin-guide/commands.md) and provide [metrics](/docs/validators/admin-guide/monitoring.md).
* And to perform administrative commands such as [scheduling upgrades](/docs/validators/admin-guide/network-upgrades.md) and changing log levels
* For more on that, see [commands](/docs/validators/admin-guide/commands.md)
...
### Outbound[](#outbound-1 "Direct link to Outbound")

* Stellar Core requires access to a database (PostgreSQL, for example). If that database resides on a different machine on your network, you'll need to allow that connection. You'll specify the database when you [configure](/docs/validators/admin-guide/configuring.md) Stellar Core.
...
## Kubernetes considerations[](#kubernetes-considerations "Direct link to Kubernetes considerations")
