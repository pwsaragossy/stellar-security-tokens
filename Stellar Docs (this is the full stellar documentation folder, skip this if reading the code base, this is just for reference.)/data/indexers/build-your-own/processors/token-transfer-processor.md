...
The Token Transfer Processor (TTP) is a [Go package](https://github.com/stellar/go-stellar-sdk/tree/main/ingest/processors/token_transfer) which uses the [ingest-sdk](/docs/data/indexers/build-your-own/ingest-sdk.md) to parse Stellar network transaction data and derive token transfer events. Before TTP, developers had to manually parse complex ledger data, operation results, and ledger entry changes to understand when and how value moved between accounts, contracts, and other entities on the network.
...
* Captures token movements resulting from:
  ...
  + [SEP-41](https://stellar.org/protocol/sep-41) compliant token events
...
> **Note:** TTP uses negative amounts in fee events to represent refunds rather than creating a separate refund event type. This approach maintains consistency with the CAP-67 specification while clearly indicating the direction of the fee transaction.
...
| `operationIndex` | `uint32*` | The one-based index of the operation within the transaction that caused this event as defined by [SEP-35](https://stellar.org/protocol/sep-35). This field is `nil` for transaction-level events like fees. |
...
> **Note:** The `toMuxedInfo` field is included in Transfer and Mint events when the destination uses a muxed account (M-address) and/or when transaction-level memo is set (in the case of non-smart contract transactions), providing additional routing information.

Please refer to [this](/docs/build/guides/transactions/pooled-accounts-muxed-accounts-memos.md) section for more information on muxed account/memo usage.
...
> **Note:** The `contractAddress` field is particularly important for DeFi applications as it provides the bridge between classic Stellar assets and their smart contract representations.
...
## Go API Overview[](#go-api-overview "Direct link to Go API Overview")
...
### EventsFromLedger[](#eventsfromledger "Direct link to EventsFromLedger")
...
### EventsFromTransaction[](#eventsfromtransaction "Direct link to EventsFromTransaction")
...
### EventsFromOperation[](#eventsfromoperation "Direct link to EventsFromOperation")
...
### Default Mode (Recommended)[](#default-mode-recommended "Direct link to Default Mode (Recommended)")
...
### Unified Events Stream Mode[](#unified-events-stream-mode "Direct link to Unified Events Stream Mode")
...
> **Caution:** If you configure TTP for unified events mode and then provide it ledgers without unified events, TTP will silently produce no events.
...
### Pre-Protocol 23 Ordering[](#pre-protocol-23-ordering "Direct link to Pre-Protocol 23 Ordering")
...
### Protocol 23+ Ordering[](#protocol-23-ordering "Direct link to Protocol 23+ Ordering")
...
> **Note:** TTP automatically detects the protocol version and applies the correct ordering rules. You don't need to configure this manually, but understanding the ordering differences is important for applications that depend on event sequence.
