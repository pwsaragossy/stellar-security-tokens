...
import { isPasskeySupported, createInvestorPasskeyWallet } from '@/utils/passkeyWallet';

type VerificationState = 'verifying' | 'success' | 'error' | 'creating_wallet' | 'wallet_created';

export function InvestorVerifyEmail() {
  ...
  const verifyEmail = async (verificationToken: string) => {
    try {
      setState('verifying');
      const response = await api.post('/investors/verify-email', {
  ...
  const handleCreateWallet = async () => {
    if (!investorData || !isPasskeySupported()) {
    ...
    try {
      ...
      const result = await createInvestorPasskeyWallet(
        investorData.id,
        investorData.email,
        investorData.name
      );
          ...
          {state === 'success' && (
            <>
              ...
              <p className="text-sm text-center text-muted-foreground">
                {investorData?.name}, your email has been verified.
              </p>
                ...
                <div className="w-full space-y-4">
                  <div className="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                    <p className="text-sm text-blue-800 dark:text-blue-200">
                      <strong>Next step:</strong> Create your passkey wallet to securely store your tokens.
