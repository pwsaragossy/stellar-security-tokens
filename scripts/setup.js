...
// Validate asset code is provided
if (!assetArg) {
  console.error('\\n❌ Error: Asset code is required.');
  console.error('   Usage: node scripts/setup.js --asset=YOURTOKEN [--network=testnet] [--supply=1000]');
  console.error('   Example: node scripts/setup.js --asset=REIT01 --supply=10000');
  process.exit(1);
}

const network = networkArg;
const assetSupply = parseFloat(supplyArg);
const assetCode = assetArg;
...
async function main() {
  try {
    ...
    // assetCode is already defined from CLI args at the top of the file
